...
class CrossEntropyLogCoshLoss(torch.nn.L1Loss):
    __constants__ = ['reduction','nclass','ntarget','loss_lambda','loss_gamma','quantiles','alpha']

    def __init__(self, reduction: str = 'mean', nclass: int = 1, ntarget: int = 1, loss_lambda: float = 1., loss_gamma: float = 1., quantiles: list = [], alpha: float = 1) -> None:
